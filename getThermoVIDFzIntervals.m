...
%
% Input:
%  videoFullpath: full path to thermographic video (e.g, videoFullpath='E:\Videos\WT_FCA_IR\24-CSP.avi';)
%  ffmpegBinPath: Path to ffmpeg/bin instalation folder (e.g., ffmpegBinPath='E:\ffmpeg\bin';)
%  immobilityThreshold: Threshold for immobility detection (default immobilityThreshold=0.05;)
...
% videoFullpath ='E:\NanciIRVideos\WT_FCA_IR_converted\24-CSP.avi'
% ffmpegBinPath='E:\ffmpeg\bin';
%[fzIntervals,tm,areaChange,areaMouse]=getThermoVIDFzIntervals('E:\NanciIRVideos\WT_FCA_IR_converted\24-CSP.avi','E:\ffmpeg\bin')
...
% %downsample 25Hz to 5hz video
% vidFramesAux = vidFramesAux(:,:,:,1:5:end);
...
ax(2)=subplot(5,1,1:4);hold on;
for i=1:size(vidFramesAux,4)
	im=imagesc(vidFramesAux(:,:,:,i));
    p=plot(ax(1),[tm(i),tm(i)],[0,0.02],'r');
    pause(0.02);
    delete(im);
    delete(p);
end